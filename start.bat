...
REM Default proxy backend, used only when no provider is passed. When a provider IS
REM passed, the backend is derived from its endpoint below so the launcher forwards
REM /v1/* to the real AgentBridge port (e.g. 5290) instead of a hardcoded one.
set "BACKEND=http://127.0.0.1:8787"
if defined AUTO_PROVIDER (
    ...
    for /f "usebackq delims=" %%b in (`powershell -NoProfile -ExecutionPolicy Bypass -Command "$b='%AUTO_PROVIDER%';$b=$b.Replace('-','+').Replace('_','/');$b=$b.PadRight(($b.Length+3)-band-4,'=');$j=[Text.Encoding]::UTF8.GetString([Convert]::FromBase64String($b));$ep=($j|ConvertFrom-Json).endpoint;if($ep){$u=[uri]$ep;'{0}://{1}:{2}' -f $u.Scheme,$u.Host,$u.Port}"`) do if not "%%b"=="" set "BACKEND=%%b"
